...
def do_add(
    ...
    for r in [parse_requirement(line, True) for line in editables] + [
        ...
        requirements[key] = r
    project.core.ui.echo(
        f"Adding packages to [bold cyan]{group}[/] {'dev-' if dev else ''}dependencies: "
...
def do_remove(
    ...
    project.core.ui.echo(
        f"Removing packages from [bold cyan]{group}[/] {'dev-' if dev else ''}dependencies: "
    ...
    for name in packages:
        ...
        if not matched_indexes:
            raise ProjectError(
                f"[bold green]{name}[/] does not exist in [bold cyan]{group}[/] dependencies."
            )
